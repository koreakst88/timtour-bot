...
bot.use(async (ctx, next) => {
  ...
  if (telegramUser) {
    try {
      ...
      await touchBotUser(telegramUser.id.toString())
    } catch (error) {
...
bot.action('menu:favorites', async (ctx) => {
  ...
  await ctx.reply('Open TimTour and check your favorites there.', mainMenu())
})

bot.action('menu:bookings', async (ctx) => {
  await ctx.answerCbQuery()
  await ctx.reply('Open TimTour and review your bookings in the mini app.', mainMenu())
})
...
bot.on('text', async (ctx) => {
  await ctx.reply('Use /start to open the menu or /admin if you are a manager.', mainMenu())
})
